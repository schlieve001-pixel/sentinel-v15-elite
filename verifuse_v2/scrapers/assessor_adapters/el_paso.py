...
Platform: SpatialEst — same Laravel/Bootstrap platform as Denver
Status: PROBED 2026-03-12 — identical to Denver adapter, different county slug

API:
  GET /co/elpaso/api/v1/search?term=ADDRESS → parcels
  GET /co/elpaso/api/v1/recordcard/{schedule} → owner + mailing
"""
...
async def lookup(page: Page, address: str, timeout_ms: int = 25000) -> dict | None:
    ...
    try:
        sr = await page.evaluate(
            "async (t) => { const r = await fetch(`/co/elpaso/api/v1/search?term=${encodeURIComponent(t)}&limit=5`,{headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}}); if(!r.ok) return null; return r.json(); }",
            clean,
        )
        if sr:
    ...
    if schedule:
        try:
            rec = await page.evaluate(
                "async (s) => { const r = await fetch(`/co/elpaso/api/v1/recordcard/${s}`,{headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}}); if(!r.ok) return null; return r.json(); }",
                str(schedule),
            )
            if rec:
                d = rec.get("data", rec)
