"""
VERIFUSE V2 — Boulder County Pre-Sale Foreclosure PDF Scraper (Engine #9)

Downloads and parses weekly Pre Sale List PDFs from the Boulder County
Public Trustee via GTS Search platform.

Fields: Foreclosure Number, Grantor, Address, Lender's Bid Amount,
Deficiency, Total Indebtedness.

Source: https://www.bouldercountypt.org/GTSSearch/allreports.aspx
Platform: GTS Search (bouldercountypt.org)
Sales: Wednesdays at 10:30 AM MT at Treasurer's Office

Usage:
  python -m verifuse_v2.scrapers.boulder_scraper
  python -m verifuse_v2.scrapers.boulder_scraper --file /path/to/local.pdf
"""
...
RAW_PDF_DIR = Path(__file__).resolve().parent.parent / "data" / "raw_pdfs" / "boulder"

GTS_BASE = "https://www.bouldercountypt.org/GTSSearch/"
REPORTS_URL = GTS_BASE + "allreports.aspx"
...
def _make_asset_id(foreclosure_number: str) -> str:
    ...
    return f"boulder_presale_{clean}"
...
def _generate_pdf_urls(weeks_back: int = 12) -> list[str]:
    """Generate candidate PDF URLs for Boulder County pre-sale lists."""
    urls = []
    ...
    for week in range(weeks_back):
        ...
        month_name = MONTH_NAMES[wed.month]
        day = wed.day
        year = wed.year

        # Pattern: "February 12, 2026 Pre Sale List.pdf"
        fname = f"{month_name} {day}, {year} Pre Sale List.pdf"
        url = GTS_BASE + "Temp/WeeklyReports/" + fname.replace(" ", "%20")
        urls.append(url)

        # Pattern without comma
        fname2 = f"{month_name} {day} {year} Pre Sale List.pdf"
        urls.append(GTS_BASE + "Temp/WeeklyReports/" + fname2.replace(" ", "%20"))

        # Misspelling patterns observed (e.g., "Novermber")
        if month_name == "November":
            fname3 = f"Novermber {day}, {year} Pre Sale List.pdf"
            urls.append(GTS_BASE + "Temp/WeeklyReports/" + fname3.replace(" ", "%20"))

    return urls


def download_presale_pdfs(weeks_back: int = 12) -> list[Path]:
    """Download Pre Sale List PDFs from Boulder County."""
    RAW_PDF_DIR.mkdir(parents=True, exist_ok=True)
    downloaded = []

    # Method 1: Scrape reports page
    try:
    ...
    if not downloaded:
        ...
        if existing:
            log.info("Using %d existing Boulder PDFs on disk", len(existing))
            return existing
        log.warning("No Boulder County pre-sale PDFs available")

    return downloaded


def parse_presale_pdf(pdf_path: str | Path) -> list[dict]:
    """Parse a Boulder County Pre Sale List PDF.

    Boulder PDFs use GTS format:
        Foreclosure: #: BLD202500XXX
        The Grantor: Owner Name
        Street Address: 123 Main St, Boulder, CO 80302
        Lender's Bid Amount: $ 520,000.00
        Deficiency: $ 0.00
        Total Indebtedness: $ 520,000.00
    """
    ...
    if not sale_date:
        fname_match = re.search(r"(\w+)\s+(\d{1,2}),?\s+(\d{4})", pdf_path.stem)
        if fname_match:
            raw = f"{fname_match.group(1)} {fname_match.group(2)} {fname_match.group(3)}"
            sale_date = _parse_date(raw)
...
def ingest_records(records: list[dict], source_file: str = "") -> dict:
    """Ingest parsed Boulder County records into the V2 database."""
    db.init_db()
    ...
    for rec in records:
        ...
        with db.get_db() as conn:
            conn.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            """, [
                asset_id, "Boulder", "CO", "boulder_co",
                rec["foreclosure_number"], "FORECLOSURE_PRESALE",
                "boulder_public_trustee_presale",
    ...
    log.info("Boulder ingestion: %d inserted, %d updated, %d skipped",
...
def run(pdf_path: str | None = None) -> dict:
    ...
    if not paths:
        return {"error": "No Boulder County pre-sale PDFs available"}
    ...
    with db.get_db() as conn:
        conn.execute("""
            ...
            VALUES ('SYSTEM', 'SCRAPE', 'boulder_presale', ?, 'boulder_scraper', ?, ?)
        """, [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Boulder County Pre-Sale Scraper")
    ...
    print()
    print("=" * 50)
    print("  BOULDER COUNTY PRE-SALE RESULTS")
    print("=" * 50)
