-- VERIFUSE V2 — Production Schema
-- Compatible with both SQLite and PostgreSQL
-- Migrate to Supabase by running this DDL against your project

-- ── Core asset table (migrated from V1) ──────────────────────────
CREATE TABLE IF NOT EXISTS assets (
    ...
    total_indebtedness REAL DEFAULT 0.0,
...
CREATE TABLE IF NOT EXISTS users (
    ...
    attorney_verified_at TEXT,
    is_active       INTEGER NOT NULL DEFAULT 1,
    created_at      TEXT NOT NULL,
    last_login_at   TEXT
);

-- ── Unlock / credit tracking ─────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_assets_surplus ON assets(estimated_surplus);
