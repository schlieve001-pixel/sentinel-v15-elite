...
import { getUnclaimedLeads, type Lead } from "../lib/api";
...
export default function UnclaimedProperty() {
  ...
  const load = useCallback((ac?: AbortController) => {
    setLoading(true);
    setError(null);
    getUnclaimedLeads({ county: countyFilter || undefined, limit: PAGE, offset }, ac?.signal)
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", color: "#e5e7eb", fontFamily: "monospace" }}>
      ...
      <div style={{ borderBottom: "1px solid #1f2937", padding: "12px 24px", display: "flex", alignItems: "center", gap: 16, background: "#111827" }}>
        ...
        <span style={{ color: "#0ea5e9", fontWeight: 700, letterSpacing: "0.08em" }}>UNCLAIMED PROPERTY</span>
        <span style={{ fontSize: "0.75em", color: "#6b7280" }}>C.R.S. § 38-13-1304 · HB25-1224</span>
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px 16px" }}>
        ...
        {/* Statute context */}
        <div style={{ background: "#0d1117", border: "1px solid #0369a1", borderRadius: 8, padding: "14px 18px", marginBottom: 20, fontSize: "0.8em", color: "#9ca3af" }}>
          <strong style={{ color: "#0ea5e9" }}>Unclaimed Property</strong> — Surplus funds transferred to the Colorado State Treasurer after the 30-month claim window (C.R.S. § 38-13-1304).
          Attorney fee cap: <strong style={{ color: "#22c55e" }}>10% under HB25-1224</strong> (eff. June 4, 2025).
          Claim window: <strong>30 months</strong> from transfer date.
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: 40, textAlign: "center", color: "#6b7280" }}>
            <div style={{ marginBottom: 12 }}>No unclaimed property leads yet.</div>
            <div style={{ fontSize: "0.82em", color: "#4b5563" }}>
              Run the unclaimed property scraper to populate this stream.
            </div>
          </div>
        ) : (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82em" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #374151", color: "#6b7280", fontSize: "0.85em" }}>
                  ...
                  <th style={{ textAlign: "left", padding: "6px 10px" }}>CLAIM DEADLINE</th>
              ...
              <tbody>
                {leads.map(lead => (
                  <tr key={lead.asset_id} style={{ borderBottom: "1px solid #1f2937" }}>
                    ...
                    <td style={{ padding: "7px 10px", color: lead.deadline_passed ? "#ef4444" : lead.days_to_claim != null && lead.days_to_claim < 60 ? "#f59e0b" : "#9ca3af", fontSize: "0.9em" }}>
                      {lead.claim_deadline?.slice(0, 10) || "—"}
                      {lead.days_to_claim != null && !lead.deadline_passed && (
                        <span style={{ marginLeft: 6, fontSize: "0.85em", color: "#6b7280" }}>({lead.days_to_claim}d)</span>
                      )}
                    </td>
                    <td style={{ padding: "7px 10px" }}>
